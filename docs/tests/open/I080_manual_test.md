...
| M4 | `bash scripts/claude/tests/test_pretooluse_push_guard.sh` を実行 | `pass=N fail=0`・exit 0。全 TC-P が PASS | 稼働中セッション可 | Claude | | 決定論スクリプトの実走（auto_test の裏取り） |
| M5 | `python3 -m json.tool .claude/settings.json` ＋ allow/ask/deny を Read で確認 | JSON 有効・allow に `git push`/`git push *`・ask に `git push *` 不在・deny 維持 | 稼働中セッション可 | Claude | | TC-S の目視裏取り |
