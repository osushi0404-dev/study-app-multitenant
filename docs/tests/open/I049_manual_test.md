...
| 5 | CI（GitHub Actions）で `e2e.yml` ジョブが実行されていることを確認する | PR の Checks 一覧に「E2E Tests (Playwright)」ジョブが表示され、既存ジョブ（Backend Tests 等）とは独立して動作する | Human | - | CI 統合確認・GitHub Secrets 設定も必要 |
