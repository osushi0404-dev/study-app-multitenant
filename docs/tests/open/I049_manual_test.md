...
| 5 | `e2e/.env.e2e` に `E2E_TEST_PASSWORD` を設定した状態で `docker compose --profile e2e run --rm e2e` を実行する（command は docker-compose.yml の `npm ci && npm test` が自動適用） | 全テスト（auth / tenant-isolation / quiz-session）が PASS する | Claude | - | E2E 全テスト通過確認 |
