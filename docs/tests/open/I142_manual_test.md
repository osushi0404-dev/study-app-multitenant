...
- 使用アカウント: `e2e_user_a@example.com`（パスワードはリポジトリルート `.env.e2e` の `E2E_TEST_PASSWORD`。E2E で使用中の有効アカウント・組織 `e2e-org-a`）
...
| 8 | ブラウザで `http://localhost:3000/register/cute_school` を開き、未使用のユーザーID・メールアドレスで登録を完了する | 「登録が完了しました。メール認証を行ってからログインしてください。」が表示され、3 秒後にログイン画面へ遷移する | Human | | 正常系の退行確認 |
