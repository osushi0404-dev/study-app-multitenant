...
-->
実行コマンド（例）: 下記「決定論ゲート（自動実走）」の grep 5 件（pytest/Jest/E2E はアプリコード変更なしのため非該当）
...
| TC-06 | 失敗注入（false-green 防止・AC5）: 該当行を欠いた入力に対し TC-01〜05 の判定コマンドが非ゼロ終了する | 実装後に該当行を削除した一時コピー（mktemp）で再確認 | 計画時実証済み（下記） |

- 合否インターフェース: **合格 = exit 0**（本イシューで導入するルールを本文書の TC 自体で dogfood する。不合格=非ゼロ終了・出力値の目視比較は合否基準にしない）。
- 計画時実証（2026-07-18・plan_I122 調査結果）: TC-01〜05 の判定コマンドを**文言未追記の現状ファイル**に対して実行し、**全て exit 1（NG）**を確認済み（失敗条件で不合格になる＝false-green でない）。
...
grep -qF '| 合否基準が合格時に非ゼロ終了する' .claude/review-agents/plan-reviewer.md
