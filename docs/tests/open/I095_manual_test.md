...
| 3 | 逆方向: `study-app-multitenant`（primary）のプレーン新規セッションから `wt-harness` 配下を `Edit`/`Bash` 書込する | ブロックされる（双方向に効くこと） | プレーン default 新規セッション | Human | | TC-M3 |
| 4 | 同セッションで別 worktree 配下を `Read`/`cat`/`grep` で読み取る | 読み取りは成功する（横断書込のみ禁止・§9 の趣旨） | プレーン default 新規セッション | Human | | TC-M4 |
| 5 | 現 worktree 配下の通常の Edit/Write/Bash 書込を行う | 従来どおり成功する（誤ブロック無し） | プレーン default 新規セッション | Human | | TC-M5 |
