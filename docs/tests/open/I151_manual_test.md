...
| 4 | TC-AUTO-15（`pip freeze`）を実行し、出力を `I151_auto_test.md` の実施記録へ貼り付ける | Django が `5.2.17`、`django-guardian` と `django-extensions` が**一覧に無い**。kombu 等の推移的依存のバージョンが記録に残る | Claude | | celery が落ちた場合の切り分け材料 |
