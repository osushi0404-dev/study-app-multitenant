...
| 4 | シークレットウィンドウ（未ログイン）で `http://localhost:3000/register` を開き、科目選択欄を確認 | 科目リスト（personal 組織の科目）がセレクトに表示される。エラー表示・空のままにならない | Human | | 401 時代は科目が出ない状態だった。`/register/<組織slug>` でも同様に表示されればなお良（任意） |
