...
|---:|------|----------|--------|--------|------|
| 1 | `docs/runbooks/worktree.md` を Read で開く | ファイルが存在し、目的/用語/トラック設計/作成手順/独立セッション/セットアップ/共有・非共有/採番/引き継ぎ/現構成の各セクションが揃っている | Claude | | 自動 TC-A1〜A9 と重複だが目視で構成の通読性を確認 |
...
| 5 | `CLAUDE.md` を開き、追加された参照行の位置とリンクを確認 | 「0. 参照先」運用・ルール: リスト内に `- worktree 並行トラック運用: docs/runbooks/worktree.md` があり、リンク先が実在 | Claude | | 自動 TC-A10/A11 と重複だが位置の妥当性を目視 |
