...
-->
実行コマンド（例）: 下記「決定論ゲート（自動実走）」の 14 件（pytest/Jest/E2E はアプリコード変更なしのため非該当）
...
| TC-05 | SKILL.md に敵対的レビューステージのフローが存在（AC1/2） | `grep -q '敵対的レビューステージ' .claude/skills/code-review/SKILL.md` | OK（exit 0） |
| TC-06 | SKILL.md に周回上限（最大 3 周）が存在（AC3/7） | `grep -q '最大 3 周' .claude/skills/code-review/SKILL.md` | OK（exit 0） |
| TC-12 | SKILL.md にサブエージェント総数上限（15）が存在（AC7） | `grep -q 'サブエージェント総数 15' .claude/skills/code-review/SKILL.md` | OK（exit 0） |
| TC-13 | SKILL.md に上限到達時の打ち切り（自動 OK 禁止）が存在（AC7） | `grep -q '打ち切り' .claude/skills/code-review/SKILL.md` | OK（exit 0） |
| TC-07 | adversarial-reviewer.md に反証マンデートが存在（AC2） | `grep -q '合格判定を反証' .claude/review-agents/adversarial-reviewer.md` | OK（exit 0） |
| TC-08 | review-rules.md に自己認証の非権威化規定が存在（AC4） | `grep -q '自己レビューは判定根拠にしない' docs/runbooks/review-rules.md` | OK（exit 0） |
| TC-09 | review-rules.md の命名表に敵対的レビュー監査記録の行が存在（AC4） | `grep -q 'adversarial_review' docs/runbooks/review-rules.md` | OK（exit 0） |
| TC-10 | code-review.sh を source する既存テスト 3 本の無回帰（AC5） | `bash scripts/claude/tests/test_review_verdict.sh`・`bash scripts/claude/tests/test_review_gates.sh`・`bash scripts/claude/tests/test_review_commit_lifecycle.sh` | OK（exit 0） |
| TC-11 | 失敗注入（false-green 防止）: grep 系 TC（TC-03〜09・TC-12/13）が該当行を欠いた入力に対し非ゼロ終了する | 手順は下記「TC-11 注入手順」参照 | OK（実装後再確認 2026-07-19・センチネル除去コピー 9 件全て非ゼロ終了。計画時実証も同日完了済み: i086_tc_prerun.sh / i086_tc_prerun2.sh） |
...
- TC-11 注入手順（実装後の再確認・grep 系 TC ごとに実施）: ①`TMP=$(mktemp)` ②`grep -v '<当該 TC の判定文言>' <対象ファイル> > "$TMP"`（該当行を除去したコピーを作成） ③当該 TC の判定コマンドの対象ファイルを `"$TMP"` に replace して実行し**非ゼロ終了**を確認 ④`rm -f "$TMP"`。TC-03〜09・TC-12/13 の 9 件すべてで非ゼロ終了になれば TC-11 合格。
...
grep -q 'サブエージェント総数 15' .claude/skills/code-review/SKILL.md
grep -q '打ち切り' .claude/skills/code-review/SKILL.md
