...
| 7 | 実運用の `/close` を feature ブランチ（未追跡 issue-review 記録あり）で実行し、`git mv` が `fatal: not under version control` を起こさず完走することを確認 | close が中断せず完走し、レビュー記録が `docs/reviews/closed/` に移動・commit/push される | Human | 検証保留→`/close I069` 実行で検証 | ユーザー判断（2026-06-19）: 本イシューの `/close` 実行そのものを No.7 のライブ検証に充てる。未追跡 `I069_issue_review_*.md` が close 案B で回収され `git mv` 成功することを実地確認する。close 完了時に本欄を OK/NG で更新する |
