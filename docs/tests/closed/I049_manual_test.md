...
| 5 | `e2e/.env.e2e` に `E2E_TEST_PASSWORD` を設定した状態で `docker compose --profile e2e run --rm e2e` を実行する（command は docker-compose.yml の `npm install && npm test` が自動適用。`e2e-init` が先行して `migrate`・`seed_e2e` を実行する。`OrganizationCategory` は `accounts.0016` マイグレーションで作成済みのため `loaddata` ステップは不要） | 全テスト（auth / tenant-isolation / quiz-session）が PASS する | Claude | - | E2E 全テスト通過確認（Init Container パターン：`e2e-init` が DB 初期化、`e2e` がテスト実行） |
