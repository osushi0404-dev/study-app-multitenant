...
- **order 送信方針（採用済みの設計判断）**: 編集 PUT では usage_kind ごとに常に `*_images_order` を送信する（変更の有無を問わない）。「変更時のみ送信」最適化は、変更検知の誤りで**画像編集が無言で無視される**重大リスクを、≤10 行の軽微な DB 操作削減のために導入することになるため**あえて採用しない**（冪等で機能上正しい・正しさ優先）。バックエンドの `order is None=無変更` 経路は API 堅牢性のため温存（code-review Low の採用判断）。
...
### 提案6（/fix-loop スキル改善｜ユーザー要望・/retro で実装）

- **背景（要望）**: 現状の `/fix-loop` は手順3で対応案提示→手順4で一括承認→手順5で修正→手順6でテストという流れで、**「原因調査・対応方針の妥当性」と「修正内容の正しさ」を分けてレビューしていない**。誤った原因特定や影響調査漏れのまま実装に進むリスクがある（本イシューでも「DB だけ検証してキャッシュ経路を見落とした」「analytics 等の同型バグの存在」など、調査・方針段階での確認価値が高かった）。
- **要望の骨子**: fix-loop に **2段階のサブエージェント・レビュー**を導入する。
  1. **レビューA（原因調査＋対応方針）**: 「根本原因の調査結果」「対応方針（どのロジック/箇所をどう直すかの計画）」「修正に関わる**影響調査**の実施有無とその結果（同型バグ・他の呼び出し箇所・関連機能への波及）」を**サブエージェント**でレビュー。
  2. **レビューB（修正内容）**: 実装した修正差分を**サブエージェント**でレビュー。
  - **順次ゲート**: レビューA が **OK の場合のみ** レビューB を実施する（誤った診断・方針のまま実装レビューに進ませない）。一気通貫では実施しない。
- **予防処置（do + gate の2層）**:
  - **do層** — `.claude/skills/fix-loop/SKILL.md` を改修:
    - 手順3.5（新規）: 手順1-3 の成果（失敗分解・根本原因・対応方針）＋**影響調査の実施有無と結果**を、専用サブエージェント（例: `fix-diagnosis-reviewer`）でレビュー。観点: 根本原因の妥当性／対応方針の適切性・代替案検討／影響調査の網羅性（同型バグ・呼び出し箇所・関連機能）。**判定 OK のときのみ手順4（承認）→5（修正）へ進む**。NG は手順2へ戻る。
