...
### 4-4. docs/runbooks/worktree.md §6（修正方針）

**修正方針**: 「1 スタックずつ・人手規律」の記述を「ポート変数化により同時 up 可能・wt-new が自動オフセット」に更新。ポート表は「既定値（変数未設定時）」と明記。COMPOSE_PROJECT_NAME 分離（既定＝ディレクトリ名）は維持される旨を残す。
