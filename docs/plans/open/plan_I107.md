...
いずれも「変更が存在しない状態で確実に不合格を返す」ことを実走で確認済み。
...
- ドキュメント: `.claude/skills/issue-bootstrap/SKILL.md`、`docs/runbooks/issue-flow.md`
...
コマンド例の `--label` を2軸に変更し、「ラベル設定」を以下の内容に置き換える（既存の種別4分類は維持）:
...
### 4-4. 既存オープンイシューへのバックフィル（25件・承認済み分類表どおり）
...
2. **ステップ2: issue-bootstrap SKILL.md の step 4 更新**（4-2 の内容。ステップ1と独立・並行可）→ AT-03 参照
3. **ステップ3: issue-flow.md の2箇所更新**（4-3 の内容。ステップ1と独立・並行可、ステップ2と同一内容の反映）→ AT-04 参照
4. **ステップ4: バックフィル25件**（4-4 の一覧どおり。**ステップ1の完了が前提**）→ AT-05/AT-06/AT-07 参照
5. **ステップ5: 文書コミット・プッシュ**（SKILL.md / issue-flow.md / 計画・テスト・レビュー文書一式を feature ブランチにコミット）
...
- **自動**: docs/tests/open/I107_auto_test.md（AT-01〜AT-07。ラベル存在・文書更新・バックフィル全件一致・フィルタ動作）
...
## 9. 承認ポイント
- [ ] 計画内容（変更点/影響）: ラベル2件新設＋文書2ファイル更新＋バックフィル25件（承認済み分類表どおり）
