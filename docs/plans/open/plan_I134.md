...
| 4 | GitHub open イシュー本文 38 件（調査結果 (B) のリスト） | 変更点 3 と同じ挿入（wt-harness 対応 12 件はローカル編集後 `--body-file` 同期・残り 26 件は取得→挿入→ `--body-file` 更新）。`## 背景/目的` 見出しが無い #10/#42/#44/#220 は本文冒頭（タイトル直後）に見出しごと挿入 |
...
```
見出しが無い本文（#10/#42/#44/#220）はタイトル行（`# …`）直後に空行＋`## 背景/目的`＋空行＋ラベル 2 行を挿入する。
...
| 見出しが無い本文（#10/#42/#44/#220）はタイトル直後に `## 背景/目的` 見出しごと挿入 | **仮定で決めた**（「全 41 件」の AC を一様に満たすため。#220 の自動生成形式とも衝突しないことを確認済み） |
