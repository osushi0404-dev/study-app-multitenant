...
**スクリプト設計（要点）**:
- **終了コード規約（明示）**: `0`=整合（memo ⊆ body）／`1`=不一致あり（メモにあって本文に無いパスを列挙）／`2`=実行不可（引数フォーマット不正・イシューファイル未検出）。grill-me は `1` を「本文整合して再実行」、`2` を「非ブロックの skip 警告」として扱う。
- 引数 `ISSUE`（`I###`）を `^I[0-9]{3}$` で**検証してからパスに使う**（不一致なら警告して `exit 2`・パスを構築せず＝任意ファイルを読まない）。`docs/issues/{open,closed}/I###.md` を探索し未検出なら警告して `exit 2`。
...
**文言案**:
- P3-b（grill-me/SKILL.md・本文整合の直後）: 「本文整合の後、`bash scripts/claude/check-memo-body-paths.sh I###` を実行する。`exit 1`（不一致）の場合は列挙されたパスを本文（実装対象/影響範囲）に追記、または意図的除外を本文に明記してから再実行し、`✅` を確認してから完了する。」
- P3-c（plan-reviewer.md・backstop 観点）: 「**設計メモ**に登場するファイルパスが本文の『**実装対象**』『影響範囲』に反映されているか（grill-me を skip した場合の保険）。メモにあって本文に無いパスは指摘する。」
...
**文言案**:
- P2-a（plan-writing-rules.md）: 「規約・命名だけでなく**中核概念・セマンティクスの変更**（判定基準・『正とする対象』等）でも、その概念を参照する同一ファイル内の全箇所（停止条件・記録・後続ステップ参照）を grep で洗い出して同時更新する。イシュー本文では設計メモの決定を再掲する構造化セクションがこれに当たり、P3 ゲート（`check-memo-body-paths.sh`）がその決定論的インスタンスである。」
- P2-b（plan-reviewer.md）: 「**中核概念**（判定基準・正とする対象等）を変更する変更で、その概念を参照する同一ファイル内の全箇所が更新されているか。」
→ 検証: TC-H・TC-I
...
**文言案**:
- P1-a（plan-writing-rules.md・do）: 「指示ファイル（SKILL.md/runbook）へ条件分岐を追加する場合、対になる既存手順と同じ**粒度**（具体コマンド/ステップまで）で書き、『〜してから』等の暗黙の省略を残さない（分岐間の**パリティ**を保つ）。」
- P1-b（plan-reviewer.md・code-reviewer.md・gate）: 「新規追加の条件分岐に、対になる既存手順が明示するコマンド・ステップの**非対称**な省略がないか。」
→ 検証: TC-J・TC-K・TC-L

### ステップ5: ドッグフーディング・整合確認
**修正方針**: 完成した P3 スクリプトを実在イシュー（I068.md・I069.md）に対し実行し整合を確認。I067 の 3 件を新ルール/ゲートに当てはめて検出可能性を確認。
→ 検証: TC-N（実イシューでのドッグフード）・手動テスト No.1〜2
...
### 自動テスト（計画駆動）— 本イシューの正
`docs/tests/open/I068_auto_test.md` の TC-A〜TC-N を正とする。内訳:
...
| 引数 `^I[0-9]{3}$` 検証・read-only（セキュリティ） | **ユーザー承認済み**（2026-06-19） | close/SKILL.md のパストラバーサル防止を踏襲した安全側の設計。採用確定 |
| P1/P2 は①手続き＋gate（決定論化しない） | **イシュー明記**（設計確認メモ・retro 階層化） | I068.md |

> 仮定で未確定の設計判断はなし（セキュリティ設計はユーザー承認済み）。
