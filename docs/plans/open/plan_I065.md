...
- **期待最終状態**: 直下 = 3件（open のみ）、`closed/` = 53 + 61 = **114件**、総数保存 64+53 = 3+114 = **117件**（記録の消失なし）
- ※ 上記件数は 2026-06-17 develop ベースの参考値。**実装直前に `wc -l` で基準値を再取得**し、他イシューの `/close`・レビュー実行で増減があれば期待値を再算出する（不変量は「直下＋`closed/` の総数が移動前後で保存」「直下に残るのは open issue の記録のみ」）。
...
> **ステップ1が無い理由**: 本イシューはブランチ作成・DB/環境構築等の初期作業が不要（markdown 編集＋`git mv` のみ）のため、慣例上ステップ1に置く準備作業を省略し、メカニズム本体をステップ2から開始している。
